...
type BackupTable = { name: string; scope: BackupScope };
...
function safeIdentifier(value: string): string {
  if (!/^[A-Za-z_][A-Za-z0-9_]*$/u.test(value)) throw new Error(`Unsafe D1 identifier in backup: ${value}`);
  return `"${value}"`;
}
function stripRedactedColumns(table: string, rows: Record<string, unknown>[]) {
  const redacted = REDACTED_COLUMNS[table];
  if (!redacted) return rows;
  return rows.map((row) => Object.fromEntries(Object.entries(row).filter(([column]) => !redacted.has(column))));
}
async function rowsForTable(db: D1Database, table: BackupTable, institutionId: string): Promise<Record<string, unknown>[]> {
  const tableName = safeIdentifier(table.name);
  if (table.scope === "institution-row") return (await db.prepare(`SELECT * FROM ${tableName} WHERE id = ?`).bind(institutionId).all<Record<string, unknown>>()).results;
  if (table.scope === "user-relation") return (await db.prepare(`SELECT t.* FROM ${tableName} t JOIN users u ON u.id=t.user_id WHERE u.institution_id=?`).bind(institutionId).all<Record<string, unknown>>()).results;
  if (table.scope === "role-relation") return (await db.prepare(`SELECT t.* FROM ${tableName} t JOIN roles r ON r.id=t.role_id WHERE r.institution_id=?`).bind(institutionId).all<Record<string, unknown>>()).results;
  return (await db.prepare(`SELECT * FROM ${tableName} WHERE institution_id = ?`).bind(institutionId).all<Record<string, unknown>>()).results;
}
function hex(bytes: ArrayBuffer) { return Array.from(new Uint8Array(bytes),(value)=>value.toString(16).padStart(2,"0")).join(""); }

export async function createPrivateLogicalBackup(env: SystemTaskEnvironment, institutionId: string, updateProgress: TaskProgress): Promise<TaskResult> {
  const exported: Record<string, Record<string, unknown>[]> = {}; let rowCount = 0;
  for (const [index, table] of BACKUP_TABLES.entries()) {
    const rows = stripRedactedColumns(table.name, await rowsForTable(env.DB, table, institutionId));
    exported[table.name] = rows; rowCount += rows.length;
    await updateProgress(Math.min(92, Math.round(((index + 1) / BACKUP_TABLES.length) * 92)));
  }
  const createdAt = new Date().toISOString();
  const snapshot = {
    format: "boardops-d1-logical-backup-v1", createdAt, institutionId,
    security: {
      privateR2Object: true,
      redactedColumns: ["users.password_hash","user_sessions.token_digest","auth_challenges.secret_hash","idempotency_keys.request_hash"],
      note: "Authentication secret material is intentionally excluded from System data-export backups.",
    },
    skippedTables: [...SKIPPED_TABLES], tables: exported,
  };
  const body = JSON.stringify(snapshot); const bytes = new TextEncoder().encode(body);
  ...
  const stored = await env.FILES.put(objectKey, body, {
    ...
    customMetadata: { format: "boardops-d1-logical-backup-v1", institutionId, sha256, redacted: "true" },
  });
  if (!stored) throw new Error("R2 did not confirm the logical backup write");
  await updateProgress(99);
  return { objectKey, bytes: bytes.byteLength, sha256, rowCount, tableCount: Object.keys(exported).length, skippedTables: [...SKIPPED_TABLES], redacted: true };
}
