...
billingRoutes.post("/bills", async (c) => {
  ...
  const now = new Date();
  const month = body.month === undefined ? now.getUTCMonth() : Number(body.month);
  const year = body.year === undefined ? now.getUTCFullYear() : Number(body.year);
  if (!isValidPeriod(month, year)) {
    return c.json({ success: false, error: "month/year must identify a valid billing period" }, 400);
  }
  ...
  if (year * 12 + month >= current.year * 12 + current.month) {
    return c.json({ success: false, error: "Bills can only be generated for completed past months" }, 422);
  }
  ...
  const accountingPeriod = await c.env.DB.prepare(
    ...
    .first<{ status: string }>();
  if (accountingPeriod?.status === "CLOSED") {
    return c.json({ success: false, error: "Cannot regenerate a closed accounting period; use adjustments" }, 422);
  }
  if (accountingPeriod?.status === "CLOSING") {
    return c.json({ success: false, error: "Accounting period is currently closing" }, 422);
  }
  ...
  const snapshot = await c.env.DB.prepare(
    ...
    .first<SnapshotRow>();
  if (!snapshot) {
    return c.json({ success: false, error: `No immutable billing snapshot exists for ${periodLabel(month, year)}` }, 422);
  }
  const payload = parseSnapshotPayload(snapshot);
  if (!payload) {
    return c.json({ success: false, error: "Billing snapshot is invalid; generation blocked" }, 422);
  }
  if (payload.residents.length === 0 || snapshot.resident_count !== payload.residents.length) {
    return c.json({ success: false, error: "Billing snapshot resident count is invalid" }, 422);
  }
  const seenUsers = new Set<string>();
  for (const line of payload.residents) {
    const lineError = validateSnapshotLine(line);
    if (lineError) return c.json({ success: false, error: `Billing snapshot invalid: ${lineError}` }, 422);
    if (seenUsers.has(line.userId)) {
      return c.json({ success: false, error: "Billing snapshot contains duplicate resident lines" }, 422);
    }
    seenUsers.add(line.userId);
    const resident = await c.env.DB.prepare(
      `SELECT id FROM users WHERE id = ? AND institution_id = ? AND role = 'USER' LIMIT 1`,
    )
      .bind(line.userId, principal.institutionId)
      .first<{ id: string }>();
    if (!resident) {
      return c.json({ success: false, error: "Billing snapshot references a resident outside this institution" }, 422);
    }
  }

  let dueDate = defaultDueDate(month, year);
  if (typeof body.dueDate === "string" && body.dueDate.trim()) {
    const parsedDueDate = isoForDateKey(body.dueDate.trim());
    if (!parsedDueDate) return c.json({ success: false, error: "dueDate must use YYYY-MM-DD" }, 400);
  ...
  for (const line of payload.residents) {
    const existing = await c.env.DB.prepare(
      `SELECT * FROM bills
        WHERE institution_id = ? AND user_id = ? AND period_month = ? AND period_year = ?
        LIMIT 1`,
    )
      .bind(principal.institutionId, line.userId, month, year)
      .first<BillRow>();

    if (existing) {
      if (existing.status !== "DRAFT" || existing.deleted_on || existing.purged_at) {
        skipped += 1;
        continue;
      }
      if (existing.paid_amount_minor > 0) {
        return c.json({ success: false, error: "Cannot publish a paid draft bill" }, 422);
      }
      await c.env.DB.prepare(
        `UPDATE bills
            SET snapshot_id = ?, source = 'SNAPSHOT',
                meal_charges_minor = ?, other_charges_minor = ?, adjustments_minor = ?,
                total_amount_minor = ?, paid_amount_minor = 0, due_amount_minor = ?,
                status = 'GENERATED', due_date = ?, generated_at = ?, updated_at = ?
          WHERE id = ?`,
      )
        .bind(
          snapshot.id,
  ...
  await audit(c, principal, "BILLS_GENERATED_FROM_SNAPSHOT", "BillingSnapshot", snapshot.id, null, {
    month,
    year,
