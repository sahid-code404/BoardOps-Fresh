          ...
          {(user?.role === "ADMIN" || user?.role === "SUPER_ADMIN") && (
            <span
              className={cn(
                ...
                getGradientForName(user?.name || "User"),
              )}
            >
              Admin
            </span>
...
export function DashboardView() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    staleTime: 10_000,
    retry: 1,
    // Preserve the golden-master behavior of keeping old data visible during
    // background refreshes, while still exposing an explicit error state when
    // the first request genuinely fails.
    ...
    ? data.isAdmin
      ? [
          ...
          },
          {
            ...
            value: data.kpis.todayOnCount,
            icon: Utensils,
            color: "success",
            change: `${data.kpis.todayOffCount} OFF`,
          ...
          },
          {
            ...
            icon: Wallet,
            color: "warning",
            change: `₹${data.kpis.totalExpenses.toLocaleString("en-IN")}`,
            prefix: "₹",
            route: "expenses" as const,
          },
          {
            label: "Meal Charge",
            value: data.kpis.currentMealCharge,
            icon: TrendingUp,
            color: "info",
            change: `${data.kpis.totalResidentMeals} meals`,
            prefix: "₹",
      ...
      : [
          {
            label: "Meals ON Today",
            value: data.todayMeals.filter((m) => m.status === "ON").length,
            icon: Utensils,
            color: "success",
            change: `${data.todayMeals.filter((m) => m.status === "OFF").length} OFF`,
            route: "billing" as const,
          },
          {
            label: "Pending Bills",
            value: data.kpis.pendingBills,
            icon: Receipt,
            color: "warning",
            change: "view billing",
            route: "billing" as const,
          ...
          },
          {
            ...
            value: data.trend.reduce((sum, point) => sum + point.on, 0),
            icon: Activity,
            color: "info",
            change: "7-day total",
            route: "billing" as const,
  ...
  return (
    <StaggerGroup className="space-y-4">
      {/* Golden-master composition: greeting, KPIs, recent activity. Account
          details belong in the persistent profile affordance/Profile screen,
          not in an extra Dashboard card. */}
      <GreetingCard />
        ...
        <StaggerItem>
          <GlassCard className="p-6 text-center" hover={false}>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              The dashboard summary could not be fetched. Your signed-in session remains available from the profile control.
            </p>
            <button
              type="button"
                    ...
                    <GlassCard className="p-4 cursor-pointer" glow={kpi.color as never}>
                      <div className="flex items-start justify-between mb-3">
                        <div
                          ...
                          style={{
                            background: `color-mix(in oklch, var(--${token}) 15%, transparent)`,
                          }}
                        >
